...
def add_battle_pages(c: canvas.Canvas, styles: dict[str, ParagraphStyle]) -> None:
    ...
    draw_cards(c, styles, [
        ...
        ("동기화", "명칭 변경", "갱신 기록과 본문 명칭을 실제 열거값과 대조."),
        ("실행", "연쇄 행동", "협동 공격, 추가 행동, 후속 스킬의 중복·재귀 제한 확인."),
    ], MARGIN_X, 376, CONTENT_W, columns=2, row_height=125)
    c.showPage()
...
def add_combat_presentation_pages(c: canvas.Canvas, styles: dict[str, ParagraphStyle]) -> None:
    ...
    draw_combat_slide(
        ...
        [
            ...
            ("픽토그래피", "이동 없음 · 원격 검 제어"),
            ("컷씬", "사용 안 함"),
            ("추가 프랍 모델링", "기존 무기 모델링을 그대로 메시만 활용해서 이펙트로 사용"),
    ...
    draw_combat_slide(
        ...
        "상태별 시퀀스",
        (
            ("Character_stun_idle.anim", "진입 시 상체와 팔을 짧게 경직시킨다. 발을 고정한 채 머리와 흉곽을 작은 범위로 흔들고, 해제 시 중심을 세워 대기 모션으로 블렌드한다."),
...
def add_tower_pages(c: canvas.Canvas, styles: dict[str, ParagraphStyle]) -> None:
    ...
    y = draw_table(c, styles, MARGIN_X, 378,
        ...
        [
            ...
            ["0턴 클리어", "전투 시작 효과가 없어 성립하지 않는 결과.", "전투 시작 효과 추가 시 조건 재검토."],
            ["빈 편성", "유닛 없이 클리어한 결과를 비정상 처리.", "시작 편성과 결과 편성의 일치."],
            ["미션 결과", "미션 성공 판정과 달성 단계는 클라이언트에서 계산해 서버로 결과만 패킷을 전송.", "서버 재검증 규칙은 별도 정의."],
